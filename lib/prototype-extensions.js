...
Array.prototype.mul = function mul(predicate = value => value) {
  return this.reduce((acc, value, i) => acc * predicate(value, i), 1);
};
...
Array.prototype.avg = function avg(predicate = value => value) {
...
String.prototype.toGrid = function toGrid(rowSeparator = '\n', columnSeparator = '', mapValue = value => value) {
  return this.split(rowSeparator).map(row => row.split(columnSeparator).map(mapValue));
};

// Convert string to array of numbers, given a separator
String.prototype.toNumArray = function toNumArray(separator = '') {
